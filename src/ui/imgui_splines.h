...
namespace ImGui
{
	...
	inline int CurveEditor(const char* label
		, float* values
		, int points_count
    ...
    inline int Spline(const char* label, float P[5])
    {
        ...
        DrawList->AddLine(
            ImVec2(bb.Min.x + 10, bb.Min.y + 10), 
            ImVec2(bb.Min.x + 100, bb.Min.y + 100), 
            ImColor(white), 3
        );

        DrawList->AddCircleFilled(
            ImVec2(bb.Min.x + 50, bb.Min.y + 50), 
            10, ImColor(cyan));
