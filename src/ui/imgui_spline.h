...
namespace ImGui
{
    ...
    inline float BezierValueNormalize(float x, float min, float max, float P[4]) {
        enum { STEPS = 256 };
        float range = (max - min);
        float r = (x - min) / range;
        ImVec2 Q[4] = { { 0, 0 }, { P[0], P[1] }, { P[2], P[3] }, { 1, 1 } };
        ImVec2 results[STEPS + 1];
        bezier_table<STEPS>(Q, results);
        float res = results[(int)((r < 0 ? 0 : r > 1 ? 1 : r) * STEPS)].y;
    ...
    inline int Bezier(const char *label, float P[5]) {
        ...
        //hovered |= 0 != ItemHoverable(ImRect(bb.Min, bb.Min + ImVec2(avail, dim)), id, ImGui::GetCurrentContext()->LastItemData.ItemFlags);
        hovered |= 0 != ItemHoverable(ImRect(bb.Min, bb.Min + ImVec2(avail, dim)), id, ImGui::GetCurrentContext()->LastItemData.ItemFlags);
