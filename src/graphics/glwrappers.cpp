...
//---------------------------------------------------------------------
QOpenGLExtraFunctions* GlCommon::gl() {
    return surface_->gl();
}
...
void ComputeShader::setup_from_source(QString shader_source, GlContext* surface)
{
    setup_surface(surface);

    activate_context();
    //Load compute shader
    xassert(program_.addShaderFromSourceCode(QOpenGLShader::Compute, shader_source),
        "Can't load shader file " + shader_source);
